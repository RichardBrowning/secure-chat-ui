...
function ChatPage({ isXs, selectedChatId, setSelectedChatId }) {
	// const messageList = [
	...
	return (
		<Container fluid className={'vh-100 d-flex flex-column'} style={{ backgroundColor: BG_COLOR }}>
			...
			<div className="p-3 overflow-auto flex-grow-1">
				...
				{sessionStorage.getItem('messageList') ? JSON.parse(sessionStorage.getItem('messageList')).map((messageBlock, index) => (
					<MessageBubble key={index} messageText={messageBlock.message} isFirstPerson={messageBlock.isFirstPerson} />
				)) : null }
			</div>
